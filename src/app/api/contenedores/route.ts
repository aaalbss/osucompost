// /app/api/contenedores/route.ts
import { NextRequest, NextResponse } from 'next/server';

const API_BASE_URL = 'http://82.165.142.177:8083/api';

// Obtener contenedores (todos o filtrados por puntoRecogidaId)
export async function GET(request: NextRequest) {
  try {
    const url = new URL(request.url);
    const searchParams = url.search; // Incluye los parámetros de consulta
    
    const targetUrl = `${API_BASE_URL}/contenedores${searchParams}`;
    ...
    console.error('Error en proxy GET contenedores:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const targetUrl = `${API_BASE_URL}/contenedores`;
    ...
    console.error('Error en proxy POST contenedor:', error);
    return NextResponse.json(
      { error: `Error en el proxy: ${error instanceof Error ? error.message : 'Error desconocido'}` },
      { status: 500 }
    );

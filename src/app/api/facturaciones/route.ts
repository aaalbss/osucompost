...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error en proxy GET facturaciones:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const targetUrl = `${API_BASE_URL}/facturaciones`;
    ...
    console.error('Error en proxy POST facturación:', error);
    return NextResponse.json(
      { error: `Error en el proxy: ${error instanceof Error ? error.message : 'Error desconocido'}` },
      { status: 500 }
    );

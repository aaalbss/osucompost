...
export async function GET(request: NextRequest) {
  try {
    ...
    const targetUrl = `${API_BASE_URL}/recogidas${searchParams}`;
    ...
    console.error('Error en proxy GET recogidas:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const targetUrl = `${API_BASE_URL}/recogidas`;
    ...
    console.error('Error en proxy POST recogida:', error);
    return NextResponse.json(
      { error: `Error en el proxy: ${error instanceof Error ? error.message : 'Error desconocido'}` },
      { status: 500 }
    );
